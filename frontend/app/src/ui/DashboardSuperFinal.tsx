      ...
      <section className="panel" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
        <div className="panel__head" style={{ cursor: 'move' }}>
          ...
          <div style={{ display: 'flex', gap: '5px' }}>
            <button onClick={() => setSiemPageState(p => Math.max(0, p - 1))} disabled={siemPageState === 0} style={{ background: 'none', border: '1px solid var(--line)', color: 'var(--signal)', padding: '2px 6px', cursor: 'pointer' }}>◄</button>
            <span style={{ fontSize: '10px', color: 'var(--text-dim)' }}>{siemPageState + 1}/{totalPages}</span>
            <button onClick={() => setSiemPageState(p => Math.min(totalPages - 1, p + 1))} disabled={siemPageState >= totalPages - 1} style={{ background: 'none', border: '1px solid var(--line)', color: 'var(--signal)', padding: '2px 6px', cursor: 'pointer' }}>►</button>
          </div>
        </div>
